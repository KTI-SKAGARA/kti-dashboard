...
- `src/middleware.ts` protects every route except `/login`, `/api/auth/logout`, `/_next`, favicon, logo, manifest, sw.js: checks Supabase session via `src/lib/supabase/middleware.ts` (`updateSession`). No more hardcoded secret (PRD P0-2 — `SESSION_SECRET` dan `admin_session` cookie legacy dihapus).
- **Auth = Supabase Auth** (email + password per-user, whitelist). Tab `profiles` di Supabase menyimpan `role` (`admin`/`viewer`) dan `is_active`. User dikelola admin-only di `/admin` (`addUser` memakai `createAdminClient` dengan `SUPABASE_SERVICE_ROLE_KEY` — jangan pernah di-import di client components). Trigger `on_auth_user_created` otomatis mengisi profiles. Session dikelola `@supabase/ssr` (cookie `sb-<ref>-auth-token`); `loginAdmin(email, password)` cek `is_active` setelah sign-in.
